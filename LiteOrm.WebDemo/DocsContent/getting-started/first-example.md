...
>
> 如果连 DI 容器都不需要，请参考 [第一个完整示例（手动构造，无 DI）](./first-example-manual.md)；如果你使用 ASP.NET Core 且需要 Autofac 集成、AOP 事务/权限/日志等能力，请参考 [第一个完整示例（DI 版）](./first-example-di.md)。
...
完整的参数表、多数据源配置、事务用法与逐段讲解见 [第一个完整示例（手动构造，无 DI）](./first-example-manual.md)。
