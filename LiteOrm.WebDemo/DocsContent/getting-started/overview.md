...
1. 先完成 [安装与环境要求](./installation.md)。
2. 使用 DI 集成时，阅读 [配置参考](../reference/configuration-reference.md)。
3. 根据项目类型跑通第一个示例：[仅基础库](./first-example.md) 或 [DI 版](./first-example-di.md)。
4. 然后进入 [实体映射与数据源](../core-usage/entity-mapping.md)、[查询总览](../core-usage/query-overview.md) 与 [Lambda 查询指南](../core-usage/lambda-guide.md)。
...
└── docs/                    # 文档
    ...
    ├── extensibility/    # 扩展性
    ├── reference/        # 参考
    └── di/        # DI 使用（依赖 LiteOrm.DependencyInjection）
```
