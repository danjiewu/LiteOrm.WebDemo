...
> Domestic databases inherit behavior from their mainstream counterparts and are auto-detected with priority. See [Database Compatibility Notes](../reference/database-compatibility.en.md) for details.

> For older database versions where default pagination syntax is incompatible, refer to [Custom Paging](../advanced-topics/custom-paging.en.md) and [Custom SqlBuilder / Dialect Extension](../extensibility/custom-sqlbuilder.en.md).
...
> When configuring `appsettings.json`, the `Provider` field requires the fully qualified type name of the database driver. See [Database Compatibility Notes](../reference/database-compatibility.en.md).
...
- You can use it with no DI container at all via `LiteOrmContext` (see [First Full Example (Manual, No DI)](./first-example-manual.en.md)), or go through plain MS DI with `AddLiteOrm()` (see [First Full Example (Base Library Only)](./first-example.en.md)).
...
- **Base only**: use DAO directly for data access; see [Entity Mapping and Data Sources](../core-usage/entity-mapping.en.md) and [Query Overview](../core-usage/query-overview.en.md).
- **LiteOrm.DependencyInjection integration**: call `RegisterLiteOrm()` during host startup; see [Configuration Reference](../reference/configuration-reference.en.md) and [First End-to-End Example](./first-example-di.en.md).

> **SQLite quick start**: If you want to try SQLite quickly, the connection string is simply `Data Source=myapp.db`—no database server needed. See the [First End-to-End Example](./first-example-di.en.md) for a complete walkthrough.
...
- [First End-to-End Example](./first-example-di.en.md)
