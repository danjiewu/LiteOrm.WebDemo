...
> 如果你不需要 Autofac/AOP/DI，可以参考[第一个完整示例（仅基础库）](./first-example.md)。基础库还提供纯 MS DI 的 `AddLiteOrm()`（无 Autofac/AOP），可先用它快速接入，再平滑升级到本文的宿主集成。

> **新手提示**：本文假设你已经完成了 [安装](./installation.md) 和 [配置](../reference/configuration-reference.md)。如果你是第一次接触 LiteOrm，建议从头到尾跟着敲一遍代码，大约需要 15 分钟。本文使用 SQLite 作为演示数据库，无需额外安装数据库服务。
...
全部通过后，恭喜你！你已经成功完成了 LiteOrm 的基础接入。接下来可以继续学习 [实体映射与数据源](../core-usage/entity-mapping.md) 和 [查询总览](../core-usage/query-overview.md)。
