...
The Service layer now offers IQueryable Lambda projections `SearchAs` / `SearchOneAs` / `SearchAsAsync` / `SearchOneAsAsync`, projecting results into custom classes or anonymous types (see [Lambda Guide](../core-usage/lambda-guide.en.md#6-projection-queries-searchas--searchoneas)).

#### Computed Columns (Non-Actual Columns)

`ColumnAttribute.Expression` + `ColumnMode.Computed` support computed columns: no physical column is generated and they are excluded from inserts/updates; SELECT returns the expression result and query conditions render the expression (see [Entity Mapping](../core-usage/entity-mapping.en.md)).
