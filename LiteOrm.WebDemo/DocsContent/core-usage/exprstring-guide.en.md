...
This is the standalone reference for `ExprString`. For the overall choice between Lambda / `Expr` / `ExprString`, start with the [Query Overview](./query-overview.en.md). For Lambda usage, see the [Lambda Guide](./lambda-guide.en.md); to construct `Expr` objects themselves, see the [Expr Guide](./expr-guide.en.md).
