...
本文是 `ExprString` 的独立说明文档。如果你关心的是 Lambda / `Expr` / `ExprString` 的整体选型，请先阅读[查询总览](./query-overview.md)；Lambda 写法见 [Lambda 查询指南](./lambda-guide.md)，构造 `Expr` 对象本身见 [Expr 使用指南](./expr-guide.md)。
