...
For the Lambda forms of `Exists` / `ExistsRelated`, see [Lambda Query Guide](./lambda-guide.en.md#5-exists-and-existsrelated). This section shows the equivalent `Expr` forms.
...
In a Lambda you can embed a dynamic `Expr` with `expr.To<bool>()`. For the full approach and trade-offs, see [Mixing Lambda and Expr](./lambda-expr-mixing.en.md).
...
The C# conditional operator `a ? b : c` in a Lambda is parsed into `Expr.If(...)` and rendered as SQL `CASE`; see [Lambda Query Guide](./lambda-guide.en.md#4-the-conditional-operator-becomes-case).
...
- [Lambda Guide](./lambda-guide.en.md)
