...
For a comparison of the three query styles and selection guidance, see [Query Overview](./query-overview.en.md). For dynamic condition assembly, see the [Expr Guide](./expr-guide.en.md). For handwritten SQL in the DAO, see the [ExprString Guide](./exprstring-guide.en.md).
...
- Large-offset paging (e.g. `Skip(10000)`) suffers poor performance; prefer ID-based cursor paging — see [Performance Optimization](../advanced-topics/performance.en.md#33-pagination-optimization).
...
This kind of Lambda is first converted into `Expr.If(...)`, then rendered as SQL `CASE WHEN ... THEN ... ELSE ... END`. Multi-branch `CASE` can be built with `Expr.Case(...)` — see the [Expr Guide](./expr-guide.en.md#14-type-conversion-and-conditional-values).
...
Use this when you want to control the correlation condition yourself. `Exists<T>` is an `Expr` static method; the equivalent `Expr` form is shown in the [Expr Guide](./expr-guide.en.md#21-explicit-exists).
...
> The `SearchAs` lambda is converted into a `SelectExpr` before execution (equivalent to the `SelectExpr` usage in [Query Overview](./query-overview.en.md#3-search-vs-searchas)). Result mapping is handled by `DataReaderConverter`: unregistered plain classes / anonymous types match by "member name ↔ column alias", while registered entity types match positionally — prefer DTO / anonymous-type projections. For sharded tables, pass the shard name via the `tableArgs` parameter.
...
> For auto-mapping and serialization of JsonNode properties, see [Data Mapping & Value Conversion](../advanced-topics/data-mapping.en.md#33-jsonnode-mapping-navigation).
...
Different databases have varying levels of JSON function support — please verify against your target database. To build JSON queries manually in `Expr`, see [Expression Extension](../extensibility/expression-extension.en.md#9-json-function-extensions).
...
For details, see [Combining Lambda and Expr](./lambda-expr-mixing.en.md).
...
- [Query Overview](./query-overview.en.md)
- [Expr Guide](./expr-guide.en.md)
- [ExprString Guide](./exprstring-guide.en.md)
- [CRUD Guide](./crud-guide.en.md)
- [Associations](./associations.en.md)
- [Mixing Lambda and Expr](./lambda-expr-mixing.en.md)
- [CTE Guide](./cte-guide.en.md)
