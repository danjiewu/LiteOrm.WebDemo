...
// 接管指定方法
LambdaExprConverter.RegisterMethodHandler(typeof(string), "MyMethod", handler);
// 仅批量挂默认处理器（methodName 为 null 时按全方法批量挂默认 handler）
...
> 在 Lambda 中使用 `JsonNode` 索引器和 `GetValue<T>()` 的写法见 [Lambda 查询指南](../core-usage/lambda-guide.md#7-jsonnode-查询)。JsonNode 属性的自动映射与序列化机制见 [数据映射与值转换](../advanced-topics/data-mapping.md#33-jsonnode-映射导航)。
...
// 构造 JSON 对象（key 需用 Expr.Const 包裹为 ValueTypeExpr）
...
- 对可被外部输入影响的函数扩展，建议与 [函数验证器](./function-validator.md) 结合使用。
