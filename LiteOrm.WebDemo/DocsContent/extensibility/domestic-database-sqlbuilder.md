...
如果你的方言有独特函数翻译（例如达梦的 `SYSDATE`、`TO_DATE` 行为差异），按 [表达式扩展](./expression-extension.md) 文档注册函数处理器并写专门测试。

## 4. 函数处理器（SqlFunction）的继承查找机制

除了覆盖 `virtual` 方法外，LiteOrm 还提供了一种更轻量的扩展点：**函数处理器（FunctionSqlHandler）**。它允许你为某个具体函数名（如 `DATE_FORMAT`、`SYSDATE`）注册翻译逻辑，而无需新建子类或覆盖方法。详见 [表达式扩展](./expression-extension.md)。
...
2. 通过日志查看实际生成的 SQL，确认方言是否命中。详见 [日志与诊断](../di/logging.md)。
...
**A：** DM8 完全支持 `OFFSET ... FETCH` 语法，与 Oracle 12c+ 一致。如果是 DM7 或更老版本，可能需要降级为 `ROW_NUMBER() OVER(...)` 双层嵌套写法，参考 [Oracle 11g 自定义分页示例](../advanced-topics/custom-paging.md) 实现一个 `Dameng7Builder : DamengBuilder` 并按数据源名注册。
...
- [返回目录](../README.md)
- [自定义 SqlBuilder / 方言扩展](./custom-sqlbuilder.md)
- [自定义分页实现示例](../advanced-topics/custom-paging.md)
- [数据库差异与兼容性说明](../reference/database-compatibility.md)
