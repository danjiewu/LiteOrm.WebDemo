...
For very simple string format concatenation, the `RegisterSimpleFunctionSqlHandler` overload can still be used; but documentation examples prioritize the new `FunctionSqlHandler` form.
...
> For using `JsonNode` indexers and `GetValue<T>()` in Lambda, see the [Lambda Guide](../core-usage/lambda-guide.en.md#7-jsonnode-queries). For auto-mapping and serialization of JsonNode properties, see [Data Mapping & Value Conversion](../advanced-topics/data-mapping.en.md#33-jsonnode-mapping-navigation).
...
- For function extensions that can be affected by external input, combining with the [Function Validator](./function-validator.en.md) is recommended.
...
- [Function Validator](./function-validator.en.md)
