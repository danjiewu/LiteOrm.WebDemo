...
> Lambda 条件查询的写法与本地服务完全一致，框架会在客户端进程内将 Lambda 转换为可序列化的 `Expr` 表达式树后传输。详见 [表达式指南](../core-usage/expr-guide.md)。
...
| `LogJsonPayloads`            | `bool`                                          | 是否在日志中记录远程调用的请求/响应 JSON 报文（Debug 级别，需开启对应日志提供程序的低级别过滤），默认 `false`                                        |
...
请求与响应的 JSON 结构参见 [表达式序列化](../extensibility/expr-serialization.md) 与源码 `LiteOrm.Common/Remote/RemoteInvocationMessage.cs`。
...
| 事务             | `[Transaction]` AOP                | 不支持跨进程事务（详见 [事务指南](../di/transactions.md)） |
...
- [配置参考](../reference/configuration-reference.md) — `RegisterLiteOrm` / `AddLiteOrmRemote` 的完整说明

- [表达式指南](../core-usage/expr-guide.md) — Lambda 条件查询，远程调用同样适用

- [表达式序列化](../extensibility/expr-serialization.md) — `Expr` 表达式树的序列化机制
