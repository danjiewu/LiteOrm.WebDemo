...
插值字符串 $"..."
    ...
    │
    ├─ 格式化项是 RawSql    → 原样追加到 SQL（绕过参数化，专用于不适合参数化的动态值如 LIMIT 行数，调用方需校验）
    │
...
> `RawSql` 是 `ExprString` 的辅助标记类型（独立 `readonly struct`，不继承 `Expr`），专用于插入**不适合参数化的动态值**（如 `LIMIT`/`OFFSET` 的整数值、分页行数等）。它**绕过参数化机制**，内联动态值时调用方必须先对值进行严格校验（如限制为非负整数），绝不可拼入未经验证的用户输入。纯静态的 SQL 文本直接写在 `ExprString` 字面量中即可，无需使用 `RawSql`。详见 [ExprString 指南 - 第 8 节 插入原始 SQL](../core-usage/exprstring-guide.md#8-插入原始-sqlrawsql)。
...
- **`RawSql`** 绕过参数化机制，专用于不适合参数化的动态值（如 `LIMIT` 行数、`ASC`/`DESC` 排序方向、动态列名）；内联动态值时**调用方必须先校验**——数值类用范围校验（如非负整数），字符串/token 类用白名单校验；纯静态文本直接写字面量即可。它不被 `ExprValidator` 扫描，也不支持 Expr JSON 序列化往返
...
当允许前端通过 JSON 构造 Expr 时（[前端原生 Expr 查询](../extensibility/frontend-native-expr.md)），务必配合验证器使用。`Expr` 基类已通过 `[JsonConverter(typeof(ExprJsonConverterFactory))]` 特性自动注册序列化转换器，直接使用 `JsonSerializer.Deserialize<Expr>(json)` 即可，无需手动注册转换器：
