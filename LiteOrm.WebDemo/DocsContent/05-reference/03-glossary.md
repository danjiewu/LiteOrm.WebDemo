...
`ExprString` 的辅助标记类型（独立 `readonly struct`，不继承 `Expr`），用于在插值字符串中原样插入数据库特定的静态 SQL 片段（如方言提示、未注册函数）。其内容**绕过参数化机制**，仅限受信任的服务端静态文本，不接受用户输入；不被 `ExprValidator` 扫描，也不支持 Expr JSON 序列化往返。需要运行时参数化的可复用片段请改用 `GenericSqlExpr`。
